...
class StudentApprovedRequestsPage extends StatefulWidget {
  final String userId; // Add userId to fetch specific student's gate passes

  const StudentApprovedRequestsPage({super.key, required this.userId});

  @override
  // ignore: library_private_types_in_public_api
  _StudentApprovedRequestsPageState createState() => _StudentApprovedRequestsPageState();
}

class _StudentApprovedRequestsPageState extends State<StudentApprovedRequestsPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Approved Gate Passes'),
      ),
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance
            ...
            .where('request', isEqualTo: 1) // Only approved requests
            .snapshots(),
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            return const Center(child: Text('Error loading approved requests'));
          }
